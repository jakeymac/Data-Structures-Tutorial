class LinkedList:
    ...
    def search_list(self,value,current_node=None):
        #To begin at the head
        if current_node is None:
            current_node = self.head
        #If we've reached the end of our linked list, and even that 
        #node doesn't have the value we're looking for, then the 
        #list does not have any nodes with the value we're looking for
        if current_node == self.tail:
            if current_node.data != value:
                return False
        #If the current node being looked at does have the right value, return true
        if current_node.data == value:
            return True
        else:
            #Otherwise, call this function again on the next node in the linked list.
            return self.search_list(value,current_node.next)
